...
def test_read_axis_values_reads_row_and_skips_header():
    ...
    values = _read_axis_values(sheet, AxisConfig(row=True, index=1, header=True))
...
def test_read_axis_values_reads_column_without_header():
    ...
    values = _read_axis_values(sheet, AxisConfig(row=False, index=2, header=False))
...
def test_load_file_reads_row_dates_and_column_workers(tmp_path):
    ...
    config = ImportConfig(
        date_axis=AxisConfig(row=True, index=1, header=True),
        worker_axis=AxisConfig(row=False, index=1, header=True),
        year=2026,
        month=3,
        shift_type_labels={},
    )
...
def test_load_file_reads_column_dates_and_row_workers(tmp_path):
    ...
    config = ImportConfig(
        date_axis=AxisConfig(row=False, index=1, header=True),
        worker_axis=AxisConfig(row=True, index=1, header=True),
        year=2026,
        month=4,
        shift_type_labels={},
    )
